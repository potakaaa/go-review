...
function CompactRouteCard({
  ...
  return (
    <Card className="flex h-full flex-col p-3">
      {canView ? (
        <Link
          href={`/dashboard/routes/${route.id}${canManage ? "/edit" : ""}`}
          aria-label={`${canManage ? "Edit" : "View"} ${route.business_name}`}
        ...
        <div className="block min-h-11 flex-1 rounded-xl">{content}</div>
      )}
