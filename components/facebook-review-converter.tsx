...
import {
  ...
  type ReviewLinkConversionState,
...
/**
 * The Facebook counterpart of `GoogleReviewConverter`: an admin pastes the
 * Page link they were given — including the `/share/…` links the iOS and
 * Android apps produce — and the Reviews-tab URL is filled in for them.
 */
export function FacebookReviewConverter({
  embedded = false,
  onConverted,
  initialUrl = "",
  autoFocus = false,
  ...
  initialUrl?: string;
  /** Focus the Facebook field when this is the primary task on a screen. */
  autoFocus?: boolean;
}) {
  const [state, formAction] = useActionState<ReviewLinkConversionState, FormData>(
    convertFacebookPageLink,
    {},
  );
  const [pageUrl, setPageUrl] = useState(initialUrl);

  const currentInput = pageUrl.trim();
  const currentResult =
  ...
  const controls = (
    <>
      <div>
        <label
          htmlFor={embedded ? "facebook_url_embedded" : "facebook_url"}
        ...
        >
          {embedded ? (
            ...
            "Facebook Page or share link"
          )}
        </label>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
          <input
            id={embedded ? "facebook_url_embedded" : "facebook_url"}
            name="facebook_url"
            type="url"
            required={!embedded}
            inputMode="url"
        ...
        <p className="mt-1.5 text-xs text-subtle">
          Paste the Page&apos;s profile link or the Share link from the mobile
          app. The converter follows Facebook&apos;s redirect and builds the
          Reviews tab URL; no Graph API token is needed.
        </p>
      </div>
