...
export function StoryForm({ story }: { story?: ShopStory }) {
  ...
  // A photo larger than the server action accepts is rejected by the framework
  // before the action runs, so nothing on the server can turn it into a
  // message. Catch it here, while the writer can still choose another photo.
  ...
  return <form action={action} className="space-y-6">
    ...
    <label className="block text-sm">Photo <span className="text-muted">— JPG, PNG, WebP · up to 8 MB</span><input className={fieldClass} type="file" name="photo" accept="image/jpeg,image/png,image/webp" onChange={event => {
      const file = event.currentTarget.files?.[0];
      if (file && file.size > STORY_UPLOAD_LIMIT) {
        event.currentTarget.value = "";
        const message = `That photo is ${(file.size / 1024 / 1024).toFixed(1)} MB. Choose one under 8 MB.`;
        setPhotoError(message);
        toast.error("Photo too large", { description: message });
      } else setPhotoError(null);
    }} /><span className="mt-2 block text-xs text-muted">A new upload replaces the current photo after the story is saved.</span></label>
    ...
    <FormError>{photoError ?? state.error}</FormError><Button type="submit" disabled={pending || photoError !== null}>{pending ? <Spinner /> : null}{pending ? "Saving story…" : "Save story"}</Button>
  </form>;
}
