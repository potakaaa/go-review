...
import { PublishRouteForm } from "@/components/publish-route-form";
import {
  RouteLockBadge,
    ...
    >
      <div className="mx-auto max-w-4xl space-y-8">
        {created ? (
          <Flash
            title="Route created"
            description={`Print or write this QR code onto the card for ${route.business_name}.`}
            params={["created"]}
          />
        ) : null}
        {saved ? (
          <Flash
            title="Changes saved"
            description={`${route.business_name} is up to date.`}
            params={["saved"]}
          />
        ) : null}
        ...
        {canPublish && route.publication_status === "draft" ? (
          <PublishRouteForm id={route.id} businessName={route.business_name} />
        ) : null}
      </div>
    </RouteStateProvider>
  );
