...
export default async function OrderDetailPage({ params, searchParams }: PageProps<"/dashboard/orders/[id]">) {
  ...
  const products = [["Google standee", order.standee_quantity], ["Google tap card", order.google_card_quantity], ["Facebook tap card", order.facebook_card_quantity], ["Instagram tap card", order.instagram_card_quantity]].filter(([, quantity]) => Number(quantity) > 0);
